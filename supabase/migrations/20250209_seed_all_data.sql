-- Add unique constraint on site_images if not exists
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint 
    WHERE conname = 'site_images_page_section_label_key'
  ) THEN
    ALTER TABLE site_images ADD CONSTRAINT site_images_page_section_label_key UNIQUE (page, section, label);
  END IF;
END $$;

-- Add unique constraint on job_listings title if not exists
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint 
    WHERE conname = 'job_listings_title_key'
  ) THEN
...
ON CONFLICT (page, section, label) DO UPDATE SET
